...
void householder (matrix*A, matrix*T, matrix*H){ 
    ...
    double*A_vec = calloc_wrapper(((pow(n, 2)+n)/2),sizeof(double));
    ...
    for (int k = 0; k < n-2; k++) { 
        ...
        //step 6 - check
        double* u = calloc_wrapper(2*n,sizeof(double));
        for (int j = k; j < n; j++) {
            u[j]=0;
            for (int i = k+1 ; i < n ; i++) {
        ...
        //step 8
        double* z = calloc_wrapper(2*n,sizeof(double));
        for (int j = k; j < n; j++) {
        ...
        // calcular w
        double* w = calloc_wrapper(2*n,sizeof(double));
        for (int i = k; i < n ; i++)
